document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnVoltarQuestao = document.getElementById('btn-voltar-questao');
    ...
    let selectedAnswerIndex = null; // Resposta selecionada mas não confirmada
    // Histórico: guarda { selectedIndex, wasCorrect } por questão para permitir voltar
    let answerHistory = [];
    ...
    function loadQuestion(restoredSelectedIndex = null) {
        selectedAnswerIndex = restoredSelectedIndex;
        ...
        if (currentQuestionIndex > 0) {
            btnVoltarQuestao.classList.remove('hidden');
        } else {
            btnVoltarQuestao.classList.add('hidden');
        }

        // Botão confirmar: oculto até selecionar uma opção
        ...
        currentQuestion.options.forEach((option, index) => {
            ...
            // Restaurar seleção anterior se voltou
            if (index === restoredSelectedIndex) {
    ...
    btnConfirmar.addEventListener('click', () => {
        ...
        const wasCorrect = selectedAnswerIndex === correctIndex;

        // Salva no histórico
        answerHistory[currentQuestionIndex] = { selectedIndex: selectedAnswerIndex, wasCorrect };
    ...
    // Voltar questão
    btnVoltarQuestao.addEventListener('click', () => {
        // Desfaz o acerto/erro da questão anterior
        const prev = answerHistory[currentQuestionIndex - 1];
        if (prev && prev.wasCorrect) correctAnswersCount--;
        answerHistory.pop();

        currentQuestionIndex--;
        loadQuestion(prev ? prev.selectedIndex : null);
    });
